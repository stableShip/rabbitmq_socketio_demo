$(function () {
    ...
    function sendMessage() {
        ...
        if (message && socket.connected) {
            ...
            $.post("/message/post", {from: username, message: message}, function (result) {
                // todo 设置消息发送状态
            });
    ...
    function sendPrivateMessage() {
        ...
        if (talkTo && message && socket) {
            ...
            // tell server to execute 'new message' and send along one parameter
            $.post("/message/post", {to: talkTo, from: username, message: message}, function (result) {
                // todo 设置消息发送状态
            });
        }
    }
    ...
    $window.keydown(function (event) {
        ...
        if (event.which === 13) {
            if (username && socket) {
                ...
                username = cleanInput($usernameInput.val().trim());
                $.post("/login", {userName: username}, function (token) {
                    setUsername(token);
                });
            }
        }
